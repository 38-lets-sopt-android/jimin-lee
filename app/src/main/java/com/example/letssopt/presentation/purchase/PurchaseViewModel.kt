...
class PurchaseViewModel(
    ...
    fun addStorageItems(item: StorageItemModel) {
        viewModelScope.launch {
            ...
            if (isStored) {
                ...
                storageDao.insertStorageItems(item.toEntity())
    ...
    private fun fetchPurchaseItems() {
        _uiState.update {
            it.copy(
                purchaseItems =
                    persistentListOf(
                        StorageItemModel(id = 1L, title = "이 사랑 통역 되나요", img = R.drawable.img_home_1),
                        StorageItemModel(id = 2L, title = "이상한일5", img = R.drawable.img_home_2),
                        StorageItemModel(id = 3L, title = "하일매리", img = R.drawable.img_home_3),
                        StorageItemModel(id = 4L, title = "이 사랑 통역 되나요", img = R.drawable.img_home_1),
                    )
            )
        }
    }
